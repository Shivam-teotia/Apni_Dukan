...
} from "../constants/ProductConstanta";
...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      let link = !keyword
        ? `/api/v1/products?page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&rating[gte]=${rating}`
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`/api/v1/reviews?id=${id}`, id);
